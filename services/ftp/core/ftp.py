...
class FTP(BaseFTP):
    def __init__(self, **kwargs) -> None:
        ...
        try:
            ...
            self.temp_directory_address_for_download = kwargs.get(
                "temp_directory_address_for_download"
            )
            ...
            if "domain" in kwargs:
                ...
                self.temp_directory_address_for_download = self.domain_dictionary[
                    "temp_directory_address_for_download"
                ]
                self.file_to_download = self.domain_dictionary["file_to_download"]

                self.directory_address_for_upload = self.domain_dictionary[
                    "directory_address_for_upload"
                ]
    ...
    def download_file(self, **kwargs) -> bool:
        ...
        try:
            ...
            if "temp_directory_address_for_download" in kwargs:
                self.temp_directory_address_for_download = kwargs[
                    "temp_directory_address_for_download"
                ]
            ...
            file_to_download_size = int(
                list(
                    filter(
                        lambda x: self.file_to_download in x, list(self.ftp.mlsd(facts=["size"]))
                    )
                )[0][1]["size"]
            )
            ...
            with open(
                self.temp_directory_address_for_download + "/" + self.file_to_download, "wb"
            ) as writer:
                self.ftp.retrbinary(f"RETR {self.file_to_download}", writer.write, 8 * 1024)
    ...
    def upload_file(self, **kwargs) -> bool:
        ...
        try:
            ...
            with open(
                self.directory_address_for_upload + "/" + self.file_to_upload, "rb"
            ) as reader:
