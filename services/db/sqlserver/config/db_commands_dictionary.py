...
class DBCommandsDictionary(BaseDictionary):
    ...
    def get_dictionary(cls, **kwargs) -> dict:
        ...
        testme_version_folder = (
            kwargs.get("testme_version_folder") if "testme_version_folder" in kwargs else ""
        )
        ...
        dictionary = {
            "STANDARD": """ USE MASTER;SELECT DATABASEPROPERTYEX ('eazybusiness', 'STATUS') """,
            "AC": """ USE MASTER;SELECT user_access FROM sys.databases WHERE name = 'eazybusiness' """,
            "MU": """ USE MASTER;DECLARE @kill varchar(8000) = ''; SELECT @kill = @kill + 'kill' + CONVERT(varchar(5), session_id) + \';\' FROM sys.dm_exec_sessions WHERE database_id  = db_id(\'eazybusiness\') EXEC(@kill);ALTER DATABASE eazybusiness SET MULTI_USER """,
            "Del": """ USE AQC;TRUNCATE TABLE ListViewCheck """,
