    ...
    def get_products(self, category_id, filter_product, q, offset, limit=200, market_place=""):
        ...
        try:
            ...
            response = self.request(
                method="get",
                url=f"{self.base_url}{self.products_url}?category_ids={category_id}&limit={limit}&offset={offset}&filter={filter_product}&q={q}",
                headers={
            ...
            self.prompt_on_screen(
                f"product ids: {response['itemSummaries']}, offset: {response['offset']} from total:{response['total']}"
            )
    ...
    def get_product_ids_with_category_id(self, category_id, filter_product, q, market_place=""):
        ...
        try:
            ...
            while True:
                ...
                item_summaries, offset, total = self.get_products(category_id=category_id,offset= offset, filter_product=filter_product, q=q)
                offset += 200
