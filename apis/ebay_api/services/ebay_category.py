...
class EbayCategory(BaseEbayApi):
    ...
    def get_ebay_category_id(self, fetch_config_model: FetchConfigModel) -> str:

        try:
            category_id_candidate = {}
            category_tree_id = self.get_default_category_tree_id_with_marketplace_id(marketplace=fetch_config_model.marketplace)

            if fetch_config_model.category_name_candidate:
                ...
                for category_id, category_name in self.category_dict.items():
                    if category_name == fetch_config_model.category_name_candidate:
                        category_id_candidate[int(category_id)] = category_name

                self.prompt_on_screen(f"category id: {category_id_candidate}")

                fetch_config_model.category_id = category_id_candidate
                return category_id_candidate

            else:
                fetch_config_model.category_id = category_tree_id
