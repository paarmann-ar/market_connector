...
class WoocommerceApi(BaseWoocommerceApi):
    ...
    def convert_ebay_product_model_to_woocommerce_product_model(
        ...
        for product in ebay_product_detail_model_list:
            ...
            # rank_math_title = product["title"].strip().split(",")[0:1]
            rank_math_title = product["title"].split(",")[0].strip()

            rank_math_model = RankMathModel()
            rank_math_model.rank_math_title = rank_math_title
            # rank_math_model.rank_math_focus_keyword= woocommerce_tag_parser_model.tags
            rank_math_model.rank_math_focus_keyword = ", ".join(
                woocommerce_tag_parser_model.tags
            )
            rank_math_model.rank_math_description = product["title"]
            woocommerce_seos_model.extend(rank_math_model.for_use_in_woocommerce())
            ...
            self.woocommerce_product_models.append(
                self.woocommerce_service_provider.woocommerce_product_model(
                    ...
                    sku="",
                    price=str(round(float(product["price"]["value"]) * price_anpassen)),
                    regular_price=str(
                    ...
                    meta_data=woocommerce_seos_model,
                )
            )
